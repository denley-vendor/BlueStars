...
HBRUSH CButtonST::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	HBRUSH hbr = CButton::OnCtlColor(pDC, pWnd, nCtlColor);
	CBrush m_HollowBrush;
	m_HollowBrush.CreateStockObject(NULL_BRUSH);   

	if(nCtlColor == CTLCOLOR_BTN || nCtlColor == CTLCOLOR_STATIC)
	{
		pDC->SetTextColor(RGB(0,0,10));
	...
	// 	if (nCtlColor == IDC_STATIC_01)
	// 	{
	// 		pDC->SetBkColor(RGB(43,63,90));
	// 		return (HBRUSH)m_HollowBrush;
	// 	}
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (wndUnderMouse && wndUnderMouse->m_hWnd == m_hWnd && wndActive)
	{
		if (!m_bMouseOnButton)
		{
			...
			if(iBitmapHot != 0)
				SetBitmaps(iBitmapHot,RGB(204, 0, 255),NULL,0);
			else
				SetBitmaps(m_hBitMapHot,RGB(204, 0, 255),NULL,0);
		} // if
	} else CancelHover();
} // End of OnMouseMove
...
LRESULT CButtonST::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	if(iBitmapNormal != 0 )
		SetBitmaps(iBitmapNormal,RGB(204, 0, 255),NULL,RGB(204, 0, 255));
	else
		SetBitmaps(m_hBitmapNormal,RGB(204, 0, 255),NULL,RGB(204, 0, 255));
...
// 		{
// 			if(iBitmapClick != 0)
// 				SetBitmaps(iBitmapClick,RGB(204, 0, 255),NULL,RGB(204, 0, 255));
// 			else
// 				SetBitmaps(m_hBitMapClick,RGB(204, 0, 255),NULL,RGB(204, 0, 255));
// 			IsClickState = false;
// 		}
// 
// 	}
...
// 		captionRect.OffsetRect((centerRect.Width() - captionRect.Width() * 1.3f/*/ 2*/), (centerRect.Height() - captionRect.Height())/2);
// 		/* RFU
...
//
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut /* = NULL */, COLORREF crTransColorOut /* = 0 */)
{
...
DWORD CButtonST::SetBitmaps(int nBitmapNormal,int nBitmapHot,int nBitMapClick, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{


	iBitmapNormal = nBitmapNormal;
	iBitmapHot = nBitmapHot;
	iBitmapClick = nBitMapClick;


	return SetBitmaps(iBitmapNormal, crTransColorIn, NULL, 0);
} // End of SetBitmaps

DWORD CButtonST::SetBitmaps(HBITMAP nBitmapNormal,HBITMAP nBitmapHot,HBITMAP nBitMapClick, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
	m_hBitmapNormal = nBitmapNormal;
	m_hBitMapHot = nBitmapHot;
	m_hBitMapClick = nBitMapClick;

	return SetBitmaps(m_hBitmapNormal, crTransColorIn, NULL, 0);
}
// This function assigns bitmaps to the button.
// Any previous icon or bitmap will be removed.
//
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	...
	g_pSetLayeredWindowAttributes(GetSafeHwnd(), RGB(204, 0, 255), 0, 1 );
...
{
	//if (m_bIsPressed)
	// m_btnSupplement.SetBitmaps(IDB_BITMAP6,RGB(204, 0, 255));// 	else
